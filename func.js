class Wheel {
  constructor(numOfSections, diameter, sections) {
    this.numOfSections = numOfSections;
    this.radius = diameter / 2;
    this.sections = sections; //The section color and text definitions
...
//Define wheel number of sections
const numOfSections = 16;

//Define colors and text for every section
const sections = [
  { text: "$0", color: "#A6B8B5" },
...
let wheel = new Wheel(numOfSections, diameter, sections);
...
wheel.startBtn.addEventListener("click", function () {
  if (wheel.currentPosition === wheel.targetPosition) {
    //If wheel is not rotating

    if (!wheel.specialState) {
      //When in normal state
      winSection = getSectionIndex(wheel.numOfSections); //Choose random winning section
    } else {
      //When in special state
      winSection = specialArr[specialCount]; //Take winning section from pedefined winning sections
      specialCount++;

      if (specialCount >= specialRounds) {
        //When all rounds in special state are played
...
function animateRotation() {
  let angle = 0;
  if (wheel.currentPosition !== wheel.targetPosition) {
    //When wheel receives new target
    ...
    if (angle < 0.01) {
      //When current position is close enough to the target
